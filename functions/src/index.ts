...
// ============================================================
export const onLeaveStatusUpdate = functions.firestore
...
// ============================================================
export const onFaceEnrolled = functions.firestore
  .document('users/{userId}')
  .onUpdate(async (change: functions.Change<functions.firestore.QueryDocumentSnapshot>) => {
    const before = change.before.data();
    const after = change.after.data();
...
// ============================================================
export const onAttendanceFailed = functions.https.onCall(async (data: any) => {
...
// ============================================================
export const scheduledOvertimeCalc = functions.pubsub
  ...
  .onRun(async () => {
    ...
    for (const doc of attendanceSnap.docs) {
      const data = doc.data();
      if (!data.checkOut?.time || !data.shiftId) continue;
