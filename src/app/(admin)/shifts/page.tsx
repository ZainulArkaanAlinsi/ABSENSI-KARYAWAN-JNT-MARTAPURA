      ...
      async (snap) => {
        ...
        const withCounts = await Promise.all(
          templates.map(async (t) => {
            const empSnap = await import('firebase/firestore').then(
              ({ getDocs, query: q, collection: col, where: w }) =>
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-[12px] text-slate-400 mt-1">
              Tambahkan template shift melalui menu Jam Kerja
            </p>
          </div>
          <a
            href="/jam-kerja"
            className="px-4 py-2 bg-emerald-500 text-white text-[12px] font-bold rounded-xl hover:bg-emerald-600 transition-colors"
          >
            Buka Jam Kerja
          </a>
        </div>
              ...
              >
                ...
                <div className="flex items-center justify-between mb-5 pb-4 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <div
                      ...
                      style={{
                        backgroundColor: `${getShiftColor(activeIdx)}18`,
                        color: getShiftColor(activeIdx),
                      }}
                    >
                      {(() => {
                        const Icon = getShiftIcon(active.name, activeIdx);
                        return <Icon size={20} />;
                      })()}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-200">
                    ...
                    <p className="text-[12px] font-bold text-slate-700">
                      {active.employeeCount ?? 0} Personel
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center gap-3 bg-slate-50 rounded-xl p-4 border border-dashed border-slate-200">
                  <p className="text-[11px] font-semibold text-slate-400">
                    Ubah jadwal melalui menu Jam Kerja
                  </p>
                  <a
                    href="/jam-kerja"
