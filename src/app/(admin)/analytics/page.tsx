...
import {
  BarChart3, TrendingUp, PieChart, ArrowUpRight,
...
export default function AnalyticsPage() {
  ...
  const avgLateRate = data.totalAttendanceRecords > 0
  ...
  return (
    <div className="flex flex-col gap-5 pb-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Hadir Hari Ini',   val: data.presentToday,      icon: CheckCircle2, accent: 'text-emerald-600', bg: 'bg-emerald-100' },
          { label: 'Telat Hari Ini',   val: data.lateToday,         icon: Clock,        accent: 'text-amber-600',   bg: 'bg-amber-100'   },
          { label: 'Absen Hari Ini',   val: data.absentToday,       icon: AlertCircle,  accent: 'text-red-500',     bg: 'bg-red-100'     },
          { label: 'Total Karyawan',   val: data.totalEmployees,    icon: Users,        accent: 'text-indigo-600',  bg: 'bg-indigo-100'  },
        ].map((s, i) => (
          <motion.div
            key={s.label}
          ...
          >
            ...
            <div>
              ...
              <p className={`text-[26px] font-black leading-none tabular-nums mt-0.5 ${s.accent}`}>{s.val}</p>
            </div>
          </motion.div>
        ))}
      </div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={weeklyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="present" stroke="#10B981" strokeWidth={2.5} fill="url(#gradPresent)" dot={{ fill: '#10B981', r: 3, strokeWidth: 0 }} />
              <Area type="monotone" dataKey="late"    stroke="#F59E0B" strokeWidth={2}   fill="url(#gradLate)"    dot={{ fill: '#F59E0B', r: 3, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
          <div className="flex items-center gap-5 mt-3 px-1">
            <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500" /><span className="text-[10px] font-semibold text-slate-400">Hadir</span></div>
            <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-amber-500" /><span className="text-[10px] font-semibold text-slate-400">Telat</span></div>
          </div>
        </motion.div>
              ...
              <div className="flex-1 flex flex-col gap-1.5">
                {data.departmentDistribution.slice(0, 5).map((dept, idx) => (
                  <div key={dept.name} className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-bold text-slate-700 tabular-nums">{dept.attendance}%</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      >
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={weeklyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barSize={14} barGap={3}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
            <XAxis dataKey="day" tick={{ fontSize: 10, fill: '#94A3B8' }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 10, fill: '#94A3B8' }} axisLine={false} tickLine={false} />
            <Tooltip content={<ChartTooltip />} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Engagement Index',     val: `${data.engagementIndex}%`,  color: 'text-emerald-600', bg: 'bg-emerald-100' },
          { label: 'Ketepatan Waktu',      val: `${data.punctualityRate}%`,  color: 'text-indigo-600',  bg: 'bg-indigo-100'  },
          { label: 'Lembur Bulan Ini',     val: `${data.overtimeThisMonth}j`,color: 'text-amber-600',   bg: 'bg-amber-100'   },
        ].map((stat, i) => (
          <motion.div
            key={i}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div>
              ...
              <p className={`text-[28px] font-black leading-none tabular-nums mt-1 ${stat.color}`}>{stat.val}</p>
            </div>
