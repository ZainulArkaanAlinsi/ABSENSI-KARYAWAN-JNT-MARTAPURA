...
import {
  Activity, Search, MapPin, Clock,
  Download, UserCheck, AlertCircle, RefreshCw,
  Inbox,
} from 'lucide-react';
...
function StatCard({
  ...
  accent = 'text-emerald-600', accentBg = 'bg-emerald-100',
}: {
  label: string; value: number; icon: React.ElementType; delay: number;
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className={`text-[26px] font-black leading-none tabular-nums mt-0.5 ${accent}`}>{value}</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5 pb-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard delay={0.06} label="Total Records" value={stats.total}   icon={Activity}   accent="text-slate-700"   accentBg="bg-slate-100"    />
        <StatCard delay={0.10} label="Hadir Hari Ini" value={stats.present} icon={UserCheck}   accent="text-emerald-600" accentBg="bg-emerald-100"  />
        <StatCard delay={0.14} label="Telat Masuk"    value={stats.late}    icon={Clock}       accent="text-amber-600"   accentBg="bg-amber-100"    />
        <StatCard delay={0.18} label="Alert SOS"      value={stats.sos}     icon={AlertCircle} accent="text-red-500"     accentBg="bg-red-100"      />
      </div>
      ...
      >
        ...
        <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
          ...
          {!loading && (
            <RefreshCw size={13} className="text-slate-300 animate-spin" style={{ animationDuration: '3s' }} />
          )}
        </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-desc font-bold text-slate-800 truncate leading-tight">{r.employeeName}</p>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <MapPin size={9} className="text-slate-400 shrink-0" />
                          <p className="text-[10px] text-slate-400 font-medium truncate">{r.employeeId}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <p className="hidden sm:block text-[12px] font-semibold text-slate-600 tabular-nums">
                      {typeof r.checkIn?.time === 'string'
                        ? r.checkIn.time.slice(0, 5)
                        : '—'}
                    </p>
                    ...
                    <div>
                      ...
                      {r.isSos && (
                        <span className="ml-1 inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-black bg-red-500 text-white uppercase tracking-wider">SOS</span>
                      )}
                    </div>
                  </motion.div>
                ))}
