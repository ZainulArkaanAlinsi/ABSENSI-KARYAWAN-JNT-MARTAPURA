    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
          <button 
            onClick={() => router.back()}
            className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-900 border border-(--border-color) flex items-center justify-center text-slate-500 hover:text-rose-600 hover:border-rose-600 transition-all shadow-sm"
          >
            <ChevronLeft size={20} />
          </button>
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-black text-slate-950 dark:text-white tracking-tighter italic uppercase">Live <span className="text-rose-600">Feed</span></h1>
              <div className="px-2 py-0.5 bg-rose-600 text-white text-[8px] font-black uppercase tracking-widest rounded-full animate-pulse">Live</div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         {[
           ...
           { label: 'Terlambat', val: records.filter(r => r.status === 'late').length, color: 'text-rose-600', bg: 'bg-rose-600/5' },
        ...
        <div className="p-8 border-b border-(--border-color) flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/50 dark:bg-slate-900/50">
           ...
           <div className="relative w-full md:w-80 group">
              ...
              <input 
                ...
                className="w-full h-12 bg-white dark:bg-slate-950 border border-(--border-color) rounded-xl pl-12 pr-4 text-xs font-bold text-slate-900 dark:text-white outline-none focus:ring-4 focus:ring-rose-600/5 focus:border-rose-600/30 transition-all"
              />
           </div>
        </div>
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-black text-rose-600 uppercase tracking-widest">{r.employeeId}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                       <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${
                          ...
                          r.status === 'late' ? 'bg-rose-600/10 border-rose-600/20 text-rose-600' :
                          'bg-amber-500/10 border-amber-500/20 text-amber-500'
                       }`}>
                          <div className={`w-1.5 h-1.5 rounded-full ${r.status === 'present' ? 'bg-emerald-500' : r.status === 'late' ? 'bg-rose-600' : 'bg-amber-500'}`} />
                          <span className="text-[9px] font-black uppercase tracking-widest">{r.status}</span>
                       </div>
                    </td>
                    <td className="px-8 py-6">
                       <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-rose-600 transition-colors">
                             <ShieldCheck size={16} />
                          </div>
                    ...
                    <td className="px-8 py-6 text-right">
                       <div className="flex flex-col items-end gap-1">
                          <div className="flex items-center gap-1 text-slate-400">
                             <MapPin size={12} />
                             <span className="text-[10px] font-bold uppercase tracking-widest italic">{r.checkIn?.latitude?.toFixed(4)}, {r.checkIn?.longitude?.toFixed(4)}</span>
                          </div>
                          <p className="text-[10px] font-black text-slate-900 dark:text-white italic uppercase tracking-tighter">JNE Martapura Main Hub</p>
                       </div>
                    </td>
                  </motion.tr>
                ))}
