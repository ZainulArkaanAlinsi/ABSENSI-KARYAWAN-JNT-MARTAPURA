  ...
  if (!rule) {
    return (
      <div className="flex flex-col items-center gap-6 py-32 text-center">
        ...
        <div>
          <p className="text-xl font-black text-white uppercase tracking-tighter">Sektor Tidak Ditemukan</p>
          <p className="text-slate-500 text-sm mt-3">Domain operasional yang diminta tidak tersedia dalam sistem.</p>
        </div>
        <button
  ...
  return (
    <div className="dash-root px-1 sm:px-2">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">Pemindaian Operasional</span>
          </div>
          <h2 className="dash-page-title leading-none">Domain {rule.name}</h2>
          <p className="dash-page-sub mt-2">{format(new Date(), 'EEEE, dd MMMM yyyy', { locale: localeId })}</p>
        </motion.div>

        <div className="flex items-center gap-3">
          <button className="dash-btn-secondary">Ekspor Log</button>
          <button className="dash-btn-primary">Sinkronisasi Real-time</button>
        </div>
      </div>
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h4 className="text-sm font-black text-white uppercase tracking-wider">Manifes Kebijakan Departemen</h4>
                <p className="text-[11px] text-slate-500 font-medium tracking-wide">Batasan operasional untuk unit ini</p>
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            {[
              { icon: Clock, label: 'Jendela Masuk', val: rule.checkInTime, color: 'var(--att-present)' },
              { icon: Clock, label: 'Penjaga Keluar', val: rule.checkOutNextDay ? `${rule.checkOutTime} (+1h)` : rule.checkOutTime, color: 'var(--att-leave)' },
              { icon: AlertTriangle, label: 'Toleransi', val: `${rule.toleranceMinutes}m`, color: 'var(--att-absent)' },
            ].map(item => (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Personel Aktif', value: totalPresent, color: 'var(--att-present)', icon: CheckCircle2 },
          { label: 'Terlambat Hadir',    value: totalLate,    color: 'var(--att-late)',    icon: AlertTriangle },
          { label: 'Izin / Cuti',      value: totalLeave,   color: 'var(--att-leave)',   icon: Activity },
          { label: 'Tidak Hadir',     value: totalAbsent,  color: 'var(--att-absent)',  icon: Activity },
        ].map((s, i) => (
          <motion.div
            key={s.label}
          ...
          >
            <div className="relative z-10">
              ...
              <p className="text-3xl font-black text-white mb-1">{loading ? '—' : s.value}</p>
              <p className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500">{s.label}</p>
            </div>
            <div className="absolute inset-x-0 bottom-0 h-[2.5px]" style={{ background: s.color, opacity: 0.6 }} />
          </motion.div>
        ))}
      </div>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="dash-card overflow-hidden">
        <div className="dash-card-header mb-0 bg-white/2 -mx-px -mt-px px-6 py-5 border-b border-white/5">
          <div>
            <h3 className="dash-card-title uppercase tracking-wider text-xs">Log Transaksi Langsung</h3>
            <p className="dash-card-sub">Resolusi Absensi Individu</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-600">
                <th className="px-6 py-4">Aset Personel</th>
            ...
            <tbody className="divide-y divide-white/3">
              {loading ? (
                 Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i}><td colSpan={6} className="px-6 py-4"><div className="h-10 w-full dash-skeleton rounded-xl" /></td></tr>
                ))
              ) : records.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-20 text-center"><p className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em]">Tidak ada sinyal terdeteksi hari ini</p></td></tr>
              ) : (
                <AnimatePresence>
                  {records.map((rec, idx) => {
                    let lateDisplay = '—';
                    ...
                    return (
                      <motion.tr key={rec.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/2 text-sm">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            <div className="h-10 w-10 flex items-center justify-center rounded-xl font-black text-xs border border-white/10" style={{ background: `${rule.color}18`, color: rule.color }}>{rec.employeeName?.charAt(0)}</div>
                            <div className="min-w-0">
                              <p className="text-[14px] font-black text-white truncate">{rec.employeeName}</p>
