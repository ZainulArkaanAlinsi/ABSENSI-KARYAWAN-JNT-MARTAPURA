...
export default function CouriersPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const cs = emps.filter(
          (e) => isCourier(e.department as string) && e.isActive !== false,
        );
  ...
  const rows = useMemo(() => {
    ...
    return couriers
      .map((c) => {
        ...
        const present = mine.filter((r) => ['present', 'late', 'overtime'].includes(r.status)).length;
  ...
  const activeToday = rows.filter((r) => r.todayStatus !== 'belum' && r.todayStatus !== 'absent').length;
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-[26px] font-black tabular-nums leading-none" style={{ color: '#E31E24' }}>
              {activeToday}<span className="text-slate-300">/{rows.length}</span>
            </p>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Aktif hari ini</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center">
          ...
          <p className="text-[12px] text-slate-400 mt-1">
            Tambahkan karyawan dengan departemen pengiriman (Rider/Driver Delivery) di menu Karyawan.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-md ${meta.cls}`}>{meta.label}</span>
                </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <div className="rounded-lg bg-slate-50 px-3 py-2">
                    <p className="text-[9px] uppercase tracking-wider text-slate-400 font-bold">Hadir (bln)</p>
                  ...
                  <div className="rounded-lg bg-slate-50 px-3 py-2">
                    ...
                    <p className="text-[16px] font-black tabular-nums" style={{ color: late > 0 ? '#E31E24' : '#0f172a' }}>{late}</p>
                  </div>
                </div>
              </div>
            );
