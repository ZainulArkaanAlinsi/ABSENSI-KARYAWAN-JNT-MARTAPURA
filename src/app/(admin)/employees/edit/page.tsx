...
import {
  ArrowLeft, Save, AlertCircle, Loader2, Check,
  User, Mail, Phone, IdCard, Building2, Briefcase,
  Calendar, Clock, MapPin, ShieldCheck, ToggleRight,
} from 'lucide-react';
...
export default function EmployeeEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [emp, jks] = await Promise.all([
          getEmployee(uid),
          getJamKerjas(),
        ]);
        if (emp) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-3">
        ...
        <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest">Memuat data...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-5 pb-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <FormSection title="Identitas" icon={User} delay={0.05}>
          ...
          <FormField label="Email" icon={Mail} note="Email tidak bisa diubah (terkait Firebase Auth)">
            <input
              type="email"
...
function FormSection({
  title, icon: Icon, delay = 0, children,
}: {
...
function FormField({
  label, icon: Icon, required, note, children,
}: {
  label: string; icon: React.ElementType; required?: boolean; note?: string; children: React.ReactNode;
}) {
  return (
    <div className="flex flex-col gap-2">
      ...
      {children}
      {note && (
        <p className="text-[10px] text-slate-400 leading-relaxed pl-4">{note}</p>
      )}
    </div>
  );
}
...
function ToggleField({
  label, desc, checked, onChange, color = 'emerald', icon: Icon,
}: {
  label: string;
  desc: string;
  checked: boolean;
  ...
  const colors = {
    emerald: { bg: 'bg-emerald-500', ring: 'ring-emerald-200', tint: 'bg-emerald-50 text-emerald-600' },
  ...
  return (
    ...
    >
      ...
      <div className={`relative w-11 h-6 rounded-full transition-colors shrink-0 mt-0.5 ${checked ? colors.bg : 'bg-slate-300'}`}>
        <motion.div
